...
public class BlazumGenerator extends GeneratorBase {
    public BlazumGenerator() {
        super(Properties.of(Material.METAL, MaterialColor.COLOR_ORANGE).harvestTool(ToolType.PICKAXE).harvestLevel(2).requiresCorrectToolForDrops().strength(4.0F, 6.0F).sound(SoundType.METAL));
    }
            ...
            if (te.hasGessence()){
                if (player.getMainHandItem().getItem() == Items.AIR){
                    if (player.isCrouching()){
                        ...
                        if (te.getDelay() <= 0){
                            te.setDelay(3);
                            InventoryHelper.dropItemStack(worldIn,pos.getX(),pos.above().getY(),pos.getZ(),new ItemStack(te.getGessence().getItem(),48));
                            return ActionResultType.SUCCESS;
                        } else {
