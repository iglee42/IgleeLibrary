...
public class GhostBlockModel implements BakedModel {
    ...
    protected List<BakedQuad> render(FluidState fluidState,@Nonnull BlockState mirrorState, @Nonnull BlockState baseState, @Nonnull BakedModel model, @Nullable Direction side, @Nonnull Random rand, @Nonnull IModelData extraData) {
        ...
        if (fluidState.getType() != Fluids.EMPTY)quads.addAll(new GhostBlockFluidModel(fluidState.getType()).bake(null,null, ForgeModelBakery.defaultTextureGetter(), BlockModelRotation.X0_Y0, null,null).getQuads(fluidState.createLegacyBlock(),side,rand,extraData));
        return quads;
    }
